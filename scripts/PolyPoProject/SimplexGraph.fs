...
let private mkTriaGraph (fg : IntFileTriaGraph) =
    ...
    printfn("number of simplexes: %d") numSimplexes 
    ...
    // printfn("Atom names: %A") nameOfAtom
        ...
        (fun idx simplex ->
            ...
            if List.length pts > 1 then 
                for i in 0 .. (List.length pts - 1) do
                    // printfn("length pts - 1: %d") (List.length pts - 1)
                    let pointsOfFace = remove i pts
                    // printfn("points of face %A") pointsOfFace
                    let indexOfFace = hashConstructor.[pointsOfFace]
                    //printfn("Index of face: %d") indexOfFace
                    Faces.[idx] <- Set.union Faces.[idx] Faces.[indexOfFace]
                    // printfn("Faces: %A") Faces.[idx]
...
//MM: ErrorMsg.Logger.Debug is not working for me and has been commented out
