...
In order to ease the process, we also provide a python script that performs all the aforementioned steps. This can be invoked as follows:
    cd scripts
    python3 runme.py [experiment]
where experiment is the name of the folder containing the desired experiment. In our example, we simply run python3 runme.py triangleRB. The script also run the model checker 
PolyLogicA on the minimised model, and performs the resultTransform script. The only step left to the user is to run, if desired, the model checking procedure on the original 
poset model.